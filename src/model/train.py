...
with wandb.init(project="EXPERIENCIAS", name=f"Train Model ExecId-{args.IdExecution}", job_type="train-model") as run:
    ...
    # --- Load standardized dataset artifact from wandb ---
    ...
    # --- Log the trained model as wandb artifact ---
    trained_model_artifact = wandb.Artifact(
        name="iris-logreg-model-trained",
        type="model",
        description="Logistic Regression model trained on standardized Iris dataset",
        metadata={"accuracy": acc}
    )
    trained_model_artifact.add_file(trained_model_path)
    run.log_artifact(trained_model_artifact)
    print("📦 Modelo entrenado registrado como artifact en wandb.")
    # --- Finish wandb run ---
